...
The `docs/demo.html` page ships four **interactive terminal recordings**
(asciinema v2) — actual captured sessions, no animations. Click the
page, hit play, watch the CLI run live in your browser. The recordings
are re-generated by CI on every push from `scripts/record_demo.py`
and the page is published automatically to GitHub Pages by the
[`pages.yml`](.github/workflows/pages.yml) workflow. **Note:** GitHub
Pages needs to be enabled once in the repo's Settings → Pages panel
("Deploy from a branch" → none; the workflow publishes via the
official Pages API).
...
You can also open the recordings locally — the page is fully static and
needs no build step:

```bash
# Option 1: open the file in a browser
xdg-open docs/demo.html   # Linux
open docs/demo.html       # macOS

# Option 2: regenerate the recordings from scratch
uv run python scripts/record_demo.py /tmp/demo.cast demo
# any asciinema player can play /tmp/demo.cast
```
...
SpeakSQL is **not yet on PyPI** — install from source for now:
...
That gets you the CLI, the FastAPI service, the in-process DuckDB driver
(used by many tests), and the dev tooling (pytest, ruff). Use `uv sync`
without the extras to install only the base library, or `--all-extras`
to pull in **every** backend driver (postgres, mysql, mssql, snowflake,
bigquery, spark — including the heavy PySpark + JVM dependencies).

**Per-driver extras** (combine as needed):

```bash
pip install speaksql[postgres]   # psycopg driver (or `uv add speaksql[postgres]` once published)
...
pip install speaksql[dev]        # pytest + ruff + mypy + pytest-asyncio
```

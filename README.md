...
The page is **opt-in deployed to GitHub Pages** by the
[`pages.yml`](.github/workflows/pages.yml) workflow. To turn it on:

1. **Repo Settings → Pages → Source: "GitHub Actions"** → Save
2. **Repo Settings → Secrets and variables → Actions → Variables →
   New repository variable:** name = `PAGES_ENABLED`, value = `true`
3. Push to main (or trigger via the workflow's "Run workflow" button
   with `force = true`)

Until step 2 is done, the `pages.yml` workflow silently no-ops so the
CI tab stays green. The page is also fully static and can be opened
